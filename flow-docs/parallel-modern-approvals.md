...
1. Fournissez un nom pour votre flux, puis sélectionnez l’icône **Enregistrer** pour enregistrer le travail réalisé jusqu’à présent.

   ![enregistrer le flux](./media/parallel-modern-approvals/save.png)

> [!NOTE]
> Sélectionnez régulièrement l’icône **Enregistrer** pour enregistrer les changements dans votre flux.
> 
> 
...
1. Sélectionnez la flèche vers le bas qui se trouve entre les cartes **Obtenir le responsable (v2)** et **Start an approval** (Démarrer une approbation).
...
1. Utilisez le plus grand bouton **Nouvelle étape** situé en bas de l’intégralité du flux de travail. Les étapes que vous ajoutez avec ce bouton s’exécutent une fois que toutes les branches sont terminées.

Dans les sections suivantes, vous utilisez le petit bouton **Insérer une nouvelle étape** pour effectuer ces étapes sur chaque branche :
...
Procédez comme suit sur le côté **SI OUI** de la branche **Condition**.
   ...
   Remarque : Votre flux utilise ces étapes pour envoyer un e-mail lorsque la demande est approuvée :
...
Procédez comme suit pour mettre à jour SharePoint lorsque des décisions sont prises.
   ...
   Remarque : Veillez à effectuer ces étapes à la fois sur le **si Oui** et **si non** côtés de la branche.
...
1. Utilisez les étapes décrites précédemment pour envoyer un courrier qui résume les résultats de chaque approbation. Envoyez ce courrier à l’employé qui a demandé des congés. Votre carte peut ressembler à cet exemple :
